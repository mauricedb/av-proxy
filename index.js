...
var proxy = httpProxy.createProxyServer({
  ...
  .on('proxyReq', function (proxyReq, req, res, options) {
      if (req.method === 'OPTIONS') {
        proxyReq.setHeader('Access-Control-Request-Headers', 'authorization, x-company');
      }
  })
  .on('proxyRes', function (proxyRes, req, res) {
    ...
    //   fs.writeFile( 'D:/home/LogFiles/Application/request-' + Date.now() + '.log', JSON.stringify(log, 0, 2), function(err) {
    //     if(err) {
    //         throw err;
    //     }
    // }); 
